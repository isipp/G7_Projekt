...
public class TransactionHelper extends SQLiteOpenHelper {
    ...
    public void addTrans(String froms, int amount, String type, String categories){
        SQLiteDatabase db = this.getWritableDatabase();

        String query = "SELECT (count(*) > 0) as found FROM categories WHERE name LIKE '" + categories +  "'";
        ...
        if(type.equals("income")) {
            ...
            db.execSQL("INSERT INTO " + TABLE_NAME + " values( " + (1 + (Math.random() * (1000 - 1))) + ", " + TransactionType.INCOME.ordinal() + ", \"IncomeOne\", 100, " + amount + ", " + "'" + categories + "' "+ ", 1637487674, 1637487674, 0) ");
            db.execSQL("UPDATE " + "account "  +  "Set amount  = amount + " + amount + " where name = "+ "'" + froms + "'");
        }if (type.equals("expense")) {
            //ID FIX TODO
            db.execSQL("INSERT INTO " + TABLE_NAME + " values( " + (1 + (Math.random() * (1000 - 1))) + ", " + TransactionType.EXPENSE.ordinal() + ", \"ExpenseOne\", 100, " + amount + ", " + "'" + categories + "' "+ ", 1637487674, 1637487674, 0) ");
            db.execSQL("UPDATE " + "account "  +  "Set amount  = amount - " + amount + " where name = "+ "'" + froms + "'");
        }else{
